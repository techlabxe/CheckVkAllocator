...
// 上記のプログラムをベースに本検証用プログラムは作成しています.
...
// mode flag (vkResetDescriptorPool vs vkFreeDescriptorSets).
const bool USE_RESET_DESCRIPTOR_POOL = true;
...
VkBool32 debugCallback(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objType, uint64_t srcObject, size_t location, int32_t msgCode, const char* pLayerPrefix, const char* pMsg, void* pUserData)
{
	...
	if (flags & VK_DEBUG_REPORT_ERROR_BIT_EXT) {
		ss << "ERROR: [" << pLayerPrefix << "] Code " << msgCode << " : " << pMsg << std::endl;
	} else if (flags & VK_DEBUG_REPORT_WARNING_BIT_EXT) {
		ss << "WARNING: [" << pLayerPrefix << "] Code " << msgCode << " : " << pMsg << std::endl;
	}
...
MyVkAllocator gVkAllocator;
VkAllocationCallbacks* GetMyAllocator()
{
	static VkAllocationCallbacks callbacks = gVkAllocator;
	return &callbacks;
}
...
private:
	...
	void setupVulkan()
	{
		...
		vkCreateFence(device, &fenceCI, GetMyAllocator(), &frameFences[0]);
		vkCreateFence(device, &fenceCI, GetMyAllocator(), &frameFences[1]);
	}
	...
	void cleanup(bool fullClean)
	{
		...
		vkDestroyPipeline(device, graphicsPipeline, GetMyAllocator());
		vkDestroyRenderPass(device, renderPass, GetMyAllocator());

		for (size_t i = 0; i < swapChainImages.size(); i++) {
			vkDestroyFramebuffer(device, swapChainFramebuffers[i], GetMyAllocator());
			vkDestroyImageView(device, swapChainImageViews[i], GetMyAllocator());
		}

		vkDestroyDescriptorSetLayout(device, descriptorSetLayout, GetMyAllocator());

		if (fullClean) {
			vkDestroySemaphore(device, imageAvailableSemaphore, GetMyAllocator());
			vkDestroySemaphore(device, renderingFinishedSemaphore, GetMyAllocator());

			vkDestroyCommandPool(device, commandPool, GetMyAllocator());

			// Clean up uniform buffer related objects
			for (auto& dsPool : descriptorPool) {
				vkDestroyDescriptorPool(device, dsPool, GetMyAllocator());
			}

			vkDestroyBuffer(device, uniformBuffer, GetMyAllocator());
			vkFreeMemory(device, uniformBufferMemory, GetMyAllocator());

			// Buffers must be destroyed after no command buffers are referring to them anymore
			vkDestroyBuffer(device, vertexBuffer, GetMyAllocator());
			vkFreeMemory(device, vertexBufferMemory, GetMyAllocator());
			vkDestroyBuffer(device, indexBuffer, GetMyAllocator());
			vkFreeMemory(device, indexBufferMemory, GetMyAllocator());

			// Note: implicitly destroys images (in fact, we're not allowed to do that explicitly)
			vkDestroySwapchainKHR(device, swapChain, GetMyAllocator());
			vkDestroyFence(device, frameFences[0], GetMyAllocator());
			vkDestroyFence(device, frameFences[1], GetMyAllocator());
			vkDestroyPipelineLayout(device, pipelineLayout, GetMyAllocator());

			vkDestroyDevice(device, GetMyAllocator());
			...
			if (ENABLE_DEBUGGING) {
				...
				DestroyDebugReportCallback(instance, callback, GetMyAllocator());
			}

			vkDestroyInstance(instance, GetMyAllocator());
		}
	}
	...
	void createInstance()
	{
		...
		if (vkCreateInstance(&createInfo, GetMyAllocator(), &instance) != VK_SUCCESS) {
	...
	void createLogicalDevice()
	{
		...
		if (vkCreateDevice(physicalDevice, &deviceCreateInfo, GetMyAllocator(), &device) != VK_SUCCESS) {
	...
	void createDebugCallback()
	{
		if (ENABLE_DEBUGGING) {
			...
			if (CreateDebugReportCallback(instance, &createInfo, GetMyAllocator(), &callback) != VK_SUCCESS) {
	...
	void createSemaphores()
	{
		...
		if (vkCreateSemaphore(device, &createInfo, GetMyAllocator(), &imageAvailableSemaphore) != VK_SUCCESS ||
			vkCreateSemaphore(device, &createInfo, GetMyAllocator(), &renderingFinishedSemaphore) != VK_SUCCESS) {
	...
	void createCommandPool()
	{
		...
		if (vkCreateCommandPool(device, &poolCreateInfo, GetMyAllocator(), &commandPool) != VK_SUCCESS) {
	...
	void createVertexBuffer()
	{
		...
		vkCreateBuffer(device, &vertexBufferInfo, GetMyAllocator(), &stagingBuffers.vertices.buffer);

		vkGetBufferMemoryRequirements(device, stagingBuffers.vertices.buffer, &memReqs);
		memAlloc.allocationSize = memReqs.size;
		getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, &memAlloc.memoryTypeIndex);
		vkAllocateMemory(device, &memAlloc, GetMyAllocator(), &stagingBuffers.vertices.memory);
		...
		vkCreateBuffer(device, &vertexBufferInfo, GetMyAllocator(), &vertexBuffer);
		vkGetBufferMemoryRequirements(device, vertexBuffer, &memReqs);
		memAlloc.allocationSize = memReqs.size;
		getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &memAlloc.memoryTypeIndex);
		vkAllocateMemory(device, &memAlloc, GetMyAllocator(), &vertexBufferMemory);
		...
		vkCreateBuffer(device, &indexBufferInfo, GetMyAllocator(), &stagingBuffers.indices.buffer);
		vkGetBufferMemoryRequirements(device, stagingBuffers.indices.buffer, &memReqs);
		memAlloc.allocationSize = memReqs.size;
		getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, &memAlloc.memoryTypeIndex);
		vkAllocateMemory(device, &memAlloc, GetMyAllocator(), &stagingBuffers.indices.memory);
		...
		vkCreateBuffer(device, &indexBufferInfo, GetMyAllocator(), &indexBuffer);
		vkGetBufferMemoryRequirements(device, indexBuffer, &memReqs);
		memAlloc.allocationSize = memReqs.size;
		getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &memAlloc.memoryTypeIndex);
		vkAllocateMemory(device, &memAlloc, GetMyAllocator(), &indexBufferMemory);
		...
		vkDestroyBuffer(device, stagingBuffers.vertices.buffer, GetMyAllocator());
		vkFreeMemory(device, stagingBuffers.vertices.memory, GetMyAllocator());
		vkDestroyBuffer(device, stagingBuffers.indices.buffer, GetMyAllocator());
		vkFreeMemory(device, stagingBuffers.indices.memory, GetMyAllocator());
	...
	void createUniformBuffer()
	{
		...
		vkCreateBuffer(device, &bufferInfo, GetMyAllocator(), &uniformBuffer);
		...
		vkAllocateMemory(device, &allocInfo, GetMyAllocator(), &uniformBufferMemory);
	...
	void updateUniformData()
	{
		...
		modelMatrix = glm::rotate(glm::mat4(1.0f), angle, glm::vec3(0, 0, 1));
		...
		// Set up projection
		auto projMatrix = glm::perspective(glm::radians(45.f), swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 10.0f);
	...
	void createSwapChain()
	{
		...
		if (vkCreateSwapchainKHR(device, &createInfo, GetMyAllocator(), &swapChain) != VK_SUCCESS) {
		...
		if (oldSwapChain != VK_NULL_HANDLE) {
			vkDestroySwapchainKHR(device, oldSwapChain, GetMyAllocator());
		}
	...
	void createRenderPass()
	{
		...
		if (vkCreateRenderPass(device, &createInfo, GetMyAllocator(), &renderPass) != VK_SUCCESS) {
	...
	void createImageViews()
	{
		...
		for (size_t i = 0; i < swapChainImages.size(); i++) {
			...
			if (vkCreateImageView(device, &createInfo, GetMyAllocator(), &swapChainImageViews[i]) != VK_SUCCESS) {
	...
	void createFramebuffers()
	{
		...
		for (size_t i = 0; i < swapChainImages.size(); i++) {
			...
			if (vkCreateFramebuffer(device, &createInfo, GetMyAllocator(), &swapChainFramebuffers[i]) != VK_SUCCESS) {
	...
	VkShaderModule createShaderModule(const std::string& filename)
	{
		...
		if (vkCreateShaderModule(device, &createInfo, GetMyAllocator(), &shaderModule) != VK_SUCCESS) {
	...
	void createGraphicsPipeline()
	{
		...
		if (vkCreateDescriptorSetLayout(device, &descriptorLayoutCreateInfo, GetMyAllocator(), &descriptorSetLayout) != VK_SUCCESS) {
		...
		if (vkCreatePipelineLayout(device, &layoutCreateInfo, GetMyAllocator(), &pipelineLayout) != VK_SUCCESS) {
		...
		if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineCreateInfo, GetMyAllocator(), &graphicsPipeline) != VK_SUCCESS) {
		...
		// No longer necessary
		vkDestroyShaderModule(device, vertexShaderModule, GetMyAllocator());
		vkDestroyShaderModule(device, fragmentShaderModule, GetMyAllocator());
	}
	...
	void createDescriptorPool()
	{
		...
		for (int i = 0; i < 2; ++i) {
			if (vkCreateDescriptorPool(device, &createInfo, GetMyAllocator(), &dsPool) != VK_SUCCESS) {
				std::cerr << "failed to create descriptor pool" << std::endl;
				exit(1);
			} else {
				std::cout << "created descriptor pool" << std::endl;
			}
			descriptorPool[i] = dsPool;
		}
	}
	...
	void draw()
	{
		...
		vkWaitForFences(device, 1, &frameFences[imageIndex], VK_TRUE, UINT64_MAX);
		vkResetFences(device, 1, &frameFences[imageIndex]);

		// descriptorset allocation.
		VkDescriptorSet dsCurrent;
		VkDescriptorSetAllocateInfo allocateInfo{
			VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO, nullptr,
			descriptorPool[imageIndex],
			1, // descriptorSetCount
			&descriptorSetLayout
		};
		auto dsPoolCurrent = descriptorPool[imageIndex];
		if (USE_RESET_DESCRIPTOR_POOL) {
			vkResetDescriptorPool(device, dsPoolCurrent, 0);
		} else {
			vkFreeDescriptorSets(device, dsPoolCurrent, 1, &descriptorSet[imageIndex]);
		}

		vkAllocateDescriptorSets(device, &allocateInfo, &dsCurrent);
		descriptorSet[imageIndex] = dsCurrent;
		...
		writeDescriptorSet.dstSet = dsCurrent;
		...
		presentToDrawBarrier.image = swapChainImages[imageIndex];
		...
		vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &dsCurrent, 0, nullptr);
		...
		if (presentQueueFamily != graphicsQueueFamily) {
			...
			drawToPresentBarrier.image = swapChainImages[imageIndex];
		...
		if (vkQueueSubmit(graphicsQueue, 1, &submitInfo, frameFences[imageIndex]) != VK_SUCCESS) {
...
MyVkAllocator::~MyVkAllocator()
{
	...
	for (auto& kv : sizeMap) {
		...
		case VK_SYSTEM_ALLOCATION_SCOPE_COMMAND:
			ss << "VK_SYSTEM_ALLOCATION_SCOPE_COMMAND : "; break;
		case VK_SYSTEM_ALLOCATION_SCOPE_OBJECT:
			ss << "VK_SYSTEM_ALLOCATION_SCOPE_OBJECT : "; break;

		case VK_SYSTEM_ALLOCATION_SCOPE_CACHE:
			ss << "VK_SYSTEM_ALLOCATION_SCOPE_CACHE : "; break;
		case VK_SYSTEM_ALLOCATION_SCOPE_DEVICE:
			ss << "VK_SYSTEM_ALLOCATION_SCOPE_DEVICE : "; break;

		case VK_SYSTEM_ALLOCATION_SCOPE_INSTANCE:
			ss << "VK_SYSTEM_ALLOCATION_SCOPE_INSTANCE : "; break;
		default:
			ss << "[" << kv.first << "] : "; break;
		}
	...
	if (total > 0) {
		OutputDebugStringA("memory leak(s) found.\n");
	} else {
		OutputDebugStringA("no memory leak(s).\n");
	}
}

void* VKAPI_CALL MyVkAllocator::Allocation(void* pUserData, size_t size, size_t align, VkSystemAllocationScope scope)
{
	return static_cast<MyVkAllocator*>(pUserData)->_Allocation(size, align, scope);
}

void* VKAPI_CALL MyVkAllocator::Reallocation(void* pUserData, void* pOriginal, size_t size, size_t align, VkSystemAllocationScope scope)
{
	return static_cast<MyVkAllocator*>(pUserData)->_Reallocation(pOriginal, size, align, scope);
}

void VKAPI_CALL MyVkAllocator::Free(void* pUserData, void* pMemory)
{
	static_cast<MyVkAllocator*>(pUserData)->_Free(pMemory);
}

void* MyVkAllocator::_Allocation(size_t size, size_t align, VkSystemAllocationScope scope)
{
	auto ret = _aligned_malloc(size, align);
	memDB[ret] = Info{ size, scope };
	sizeMap[scope] += size;
	return ret;
}

void* MyVkAllocator::_Reallocation(void* pOriginal, size_t size, size_t align, VkSystemAllocationScope scope)
{
	auto oldSize = memDB[pOriginal];
	sizeMap[scope] -= oldSize.size;

	memDB.erase(pOriginal);
	auto ret = _aligned_realloc(pOriginal, size, align);
	sizeMap[scope] += size;
	memDB[ret] = Info{ size, scope };
...
void MyVkAllocator::_Free(void* pMemory)
{
	auto data = memDB[pMemory];
	sizeMap[data.scope] -= data.size;
	memDB.erase(pMemory);
